...
from route.mypage_index import MypageIndex
from route.mypage_info import MypageInfo
    ...
    if test:
        emit('ansChatbot',
             {'dialog': '문제를 모두 푸셨습니다. 채점을 시작합니다.', 'callState': 'testTermMarking',
              'filename': filename, 'test': msg['test']})
        msg['callState'] = b'testTermMarking'
        tmp = chatbot(msg)
        emit('ansChatbot',
             {'dialog': tmp['dialog'], 'callState': tmp['callState'], 'filename': filename, 'test': msg['test']})
...
api.add_resource(MypageIndex, '/mypage-index')
api.add_resource(MypageInfo, '/mypage-info')
